...
import { Input, Button, Typography, Tag, Space, Spin, Avatar, Collapse } from 'antd';
import { SendOutlined, RobotOutlined, UserOutlined, ReloadOutlined, LoadingOutlined, CheckCircleFilled, ThunderboltOutlined, CaretRightOutlined } from '@ant-design/icons';
...
// Workflow templates per agent type (simulated steps match the architecture)
...
export default function ChatWindow({ agentId, agentName, agentEmoji }: Props) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.message) {
        // Small delay so workflow animation finishes first
  ...
  const renderContent = (content: string) => {
    ...
    return lines.map((line, i) => {
      if (line.startsWith('|') && line.includes('|', 1)) {
        ...
        return (
          <div key={i} style={{ display: 'flex', gap: 0, fontFamily: 'monospace', fontSize: 12 }}>
            {cells.map((c, j) => (
              <div key={j} style={{
                flex: j === 0 ? 2 : 1,
                padding: '2px 8px',
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 160px)', gap: 0 }}>
      ...
      <div style={{ width: 200, borderRight: '1px solid #1a3055', overflowY: 'auto', padding: '12px 0', background: 'rgba(6,11,20,0.5)', flexShrink: 0 }}>
        <div style={{ padding: '0 12px', marginBottom: 8 }}>
          <Button block type="primary" ghost size="small" icon={<ReloadOutlined />} onClick={loadConversations} style={{ borderColor: '#1a3055' }}>新对话</Button>
        </div>
        {convs.map((c) => (
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#0a1628' }}>
        <div style={{ padding: '10px 20px', borderBottom: '1px solid #1a3055', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 20 }}>{agentEmoji}</span>
          <Text strong style={{ color: '#e8edf5' }}>{agentName}</Text>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          {messages.length === 0 ? (
            ...
            messages.map((msg) => (
              <div key={msg.id} style={{ marginBottom: 20, display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', animation: 'fadeInUp 0.3s ease' }}>
                {msg.role === 'assistant' && (
                  <Avatar size={32} style={{ background: 'rgba(0,180,255,0.12)', color: '#00b4ff', marginRight: 10, flexShrink: 0 }} icon={<RobotOutlined />} />
                )}
                <div className="chat-bubble" style={{
                  ...
                  <div style={{ fontSize: 14, whiteSpace: 'pre-wrap' }}>{renderContent(msg.content)}</div>
                </div>
                {msg.role === 'user' && (
                  <Avatar size={32} style={{ background: 'rgba(52,211,153,0.12)', color: '#34d399', marginLeft: 10, flexShrink: 0 }} icon={<UserOutlined />} />
                )}
              </div>
            ))
          )}
        ...
        <div style={{ padding: '14px 20px', borderTop: '1px solid #1a3055' }}>
          <div style={{ display: 'flex', gap: 8 }}>
            <Input.TextArea
              ...
              disabled={loading}
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid #1a3055', color: '#e8edf5', borderRadius: 10 }}
            />
            <Button type="primary" icon={<SendOutlined />} onClick={handleSend} loading={loading}
              style={{ background: 'linear-gradient(135deg, #00b4ff, #0958d9)', border: 'none', borderRadius: 10, height: 'auto', padding: '8px 20px' }}>
              发送
            </Button>
          </div>
          <Text type="secondary" style={{ fontSize: 11, marginTop: 4, display: 'block' }}>⚡ 演示模式 · 模拟模型响应</Text>
        </div>
      </div>
